...
fn los_valores_del_front_panel_cruzan_la_frontera_del_host() {
    ...
    let mut v = load("../vis/suma.qvi.json");
...
fn los_arrays_viajan_por_la_memoria_lineal() {
    let panel = run_file("../vis/arrays-strings.qvi.json").unwrap();
...
fn un_array_puede_crecer_a_traves_de_las_iteraciones() {
    let panel = run_file("../vis/adquisicion.qvi.json").unwrap();
...
fn la_arena_no_crece_si_ningun_puntero_sobrevive_a_la_iteracion() {
    ...
    let pocas = run_with("../vis/arena-estable.qvi.json", "limite", 9.0);
    let muchas = run_with("../vis/arena-estable.qvi.json", "limite", 99_999.0);
...
fn la_arena_si_crece_cuando_el_dato_sobrevive() {
    ...
    let pocas = run_with("../vis/adquisicion.qvi.json", "limite", 9.0);
    let muchas = run_with("../vis/adquisicion.qvi.json", "limite", 999.0);
    assert!(
...
fn el_modulo_generado_es_wasm_valido() {
    let v = load("../vis/while-suma.qvi.json");
    let wasm = compile::compile(&v).unwrap();
